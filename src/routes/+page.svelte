<script lang="ts">
	...
	let working_skills: Skill[] = $derived(
		all_skills
			.filter((skill) => {
				if (
					!skill.tags.some((tag) => {
					...
					return false;
				if (!skillFilter.source[skill.source.toUpperCase().replaceAll(' ', '_')]) return false;
				if (
					skill.class != null &&
					!skillFilter.class[skill.class.toUpperCase().replaceAll(' ', '_')]
				)
					return false;
				if (skillFilter.sc && !skill.sc) return false;
				if (skillFilter.hc && !skill.hc) return false;
				return true;
			})
			.sort((a, b) => {
				if (a[skillFilter.sort_field] == null && b[skillFilter.sort_field] == null) {
					...
					return a[skillFilter.sort_field] < b[skillFilter.sort_field]
						? skillFilter.sort_descending
							? 1
							: -1
						: skillFilter.sort_descending
							? -1
...
<svelte:head>
	...
	<meta
		content="A glorified spreadsheet tracking permanent skills for a Kingdom of Loathing account."
...
<section>
				...
				<tr>
					<th colspan={Object.entries(SkillFields).length + 1}>
						{#each Object.entries(Classes) as classEntry}
							<button onclick={() => (skillFilter = classFilterMap[classEntry[0]])}
								>{classEntry[1]}</button
							>
						{/each}
					...
					<tr>
						...
						{#each Object.entries(SkillFields) as field}
							...
							{:else if field[1] === SkillFields.TAGS}
								<td title={skill[field[1]].toString()} style="overflow: hidden; max-width: 10em"
									>{skill[field[1]]}</td
								>
							{:else}
								<td>{skill[field[1]]}</td>
	...
	<FloatingFilter bind:skillFilter />
</section>
